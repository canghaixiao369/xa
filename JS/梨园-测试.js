var rule = {
    ...
    一级: $js.toString(()=>{
        ...
        list.forEach(it => {
            ...
            // 提取缩略图：优先使用 imgs.wh34(竖图)，其次 imgs.wh169(横图)，最后使用 img 字段
            // 图片路径格式: https://fly.daoran.tv/API_ROP/ + 相对路径
            let imgUrl = '';
            if(it.imgs && it.imgs.wh34 && it.imgs.wh34.length > 0 && it.imgs.wh34 !== 'null'){
                imgUrl = 'https://fly.daoran.tv/API_ROP/' + it.imgs.wh34;
            }else if(it.imgs && it.imgs.wh169 && it.imgs.wh169.length > 0 && it.imgs.wh169 !== 'null'){
                imgUrl = 'https://fly.daoran.tv/API_ROP/' + it.imgs.wh169;
            }else if(it.img && it.img.length > 0 && it.img !== 'null'){
                imgUrl = 'https://fly.daoran.tv/API_ROP/' + it.img;
            }else{
                imgUrl = 'https://img0.baidu.com/it/u=4079405848,3806507810&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=750';
            }
            d.push({
            url:id,
            title:it.name,
    ...
    二级: $js.toString(()=>{
        ...
        let list = json.pb.dataList;
        
        // 提取专辑封面图
        let picUrl = '';
        if(album.imgs && album.imgs.wh34 && album.imgs.wh34.length > 0 && album.imgs.wh34 !== 'null'){
            picUrl = 'https://fly.daoran.tv/API_ROP/' + album.imgs.wh34;
        }else if(album.imgs && album.imgs.wh169 && album.imgs.wh169.length > 0 && album.imgs.wh169 !== 'null'){
            picUrl = 'https://fly.daoran.tv/API_ROP/' + album.imgs.wh169;
        }else if(album.img && album.img.length > 0 && album.img !== 'null'){
            picUrl = 'https://fly.daoran.tv/API_ROP/' + album.img;
        }else{
            picUrl = 'https://img0.baidu.com/it/u=4079405848,3806507810&fm=253&fmt=auto&app=138&f=JPEG?w=500&h=750';
        }
